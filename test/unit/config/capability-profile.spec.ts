...
describe('perfil de capacidad messaging', () => {
  ...
  it('con perfil messaging y APP_ROLE=worker, el worker arranca sin JWT de usuarios ni Redis', () => {
    expect(failedPaths({ ...production, ATLAS_CAPABILITY_PROFILE: 'messaging', JWT_ACCESS_TOKEN_SECRET: DEFAULT_JWT_SECRET })).toEqual([]);
  });
