...
describe('operations.mapper', () => {
  it('toManualReviewWorkItem mapea (customerId null vs set, openedAt null)', () => {
    const base = { id: 3, caseCode: 'MR-1', priority: 'high', status: 'open', caseType: 'kyc', openedAt: null, createdAtValue: new Date('2026-01-01T00:00:00.000Z') };
    expect(toManualReviewWorkItem({ ...base, customerId: 9 } as never)).toMatchObject({ workItemType: 'manual_review', caseId: '3', customerId: '9', openedAt: null });
  ...
  it('toFraudWorkItem mapea severidad/estado/patrón', () => {
    ...
    expect(res).toMatchObject({ workItemType: 'fraud', caseId: '4', priority: 'critical', status: 'investigating', reasonCode: 'velocity' });
  });

  it('toInvestigationSummaryResponse arma risk/profile con sus ramas null', () => {
    const customer = { id: 5, customerCode: 'C1', lifecycleStatus: 'approved', primaryPhoneLast4: '1', primaryEmailDomain: 'x', createdAtValue: new Date('2026-01-01T00:00:00.000Z') };
    const withData = toInvestigationSummaryResponse({
      ...
      latestRiskResult: { riskAssessmentRunId: 7, assessmentType: 'onboarding', recommendedAction: 'approve', riskLevel: 'low', fraudScore: '0.12', decidedAt: null } as never,
