/**
 * @file Cómo se DIBUJA el informe de gastos: cabecera, totales, categorías y pie.
...
/**
 * Sale de `SpendingReportService` porque aquel archivo era un caso de uso de treinta líneas
 * —pedir los datos y devolver los bytes— con doscientas de maquetación pegadas. Son dos cosas que
 * cambian por razones distintas: una cuando cambia qué se informa, otra cuando cambia cómo se ve.
 */
@Injectable()
export class SpendingReportLayoutService {
  constructor(
   ...
   */
  header(document: PDFKit.PDFDocument, customerName: string | null, generatedAt: string): void {
  ...
  totals(document: PDFKit.PDFDocument, data: Awaited<ReturnType<LoanSpendingService['byCategory']>>): void {
    const cards = [
   ...
   */
  categories(document: PDFKit.PDFDocument, data: Awaited<ReturnType<LoanSpendingService['byCategory']>>): void {
   ...
   */
  footer(document: PDFKit.PDFDocument, data: Awaited<ReturnType<LoanSpendingService['byCategory']>>): void {
