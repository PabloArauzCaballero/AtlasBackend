...
describe('DigitalTrustGenericAdapter', () => {
  ...
  const local = (scenario?: string) =>
    ({ mode: 'local', providerCode: 'DIGITAL_TRUST_GENERIC', input: scenario ? { scenario } : {} }) as never;
  ...
  it('normalize (riesgo alto) exige revisión en device/ip/synthetic sobre el umbral', async () => {
    const obs = await adapter.normalize({
      status: 'COMPLETED',
      payload: { emailRiskLevel: 'HIGH', deviceRiskScore: 0.78, ipRiskScore: 0.84, syntheticIdentityRiskLevel: 'HIGH' },
    } as never);
    ...
    expect(obs.find((o) => o.observationKey === 'synthetic_identity_risk_level')).toMatchObject({
  ...
  it('normalize (riesgo bajo) no exige revisión', async () => {
    const obs = await adapter.normalize({
      status: 'COMPLETED',
      payload: { emailRiskLevel: 'LOW', deviceRiskScore: 0.15, ipRiskScore: 0.18, syntheticIdentityRiskLevel: 'LOW' },
    } as never);
