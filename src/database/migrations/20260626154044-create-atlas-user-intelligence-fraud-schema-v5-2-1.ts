...
function resolveColumnType(spec: ColumnSpec): ModelAttributeColumnOptions<Model>['type'] {
  switch (spec.kind) {
    ...
    case 'DECIMAL':
      return spec.precision && spec.scale !== undefined
        ? DataTypes.DECIMAL(spec.precision, spec.scale)
        : DataTypes.DECIMAL;
    case 'DATE':
...
function indexName(index: IndexSpec): string {
  const raw = index.rawColumns
    ? index.rawColumns.replace(/[^a-zA-Z0-9]+/g, '_')
    : (index.fields ?? []).join('_');
