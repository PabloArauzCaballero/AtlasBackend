...
type Claim =
  { mode: 'execute'; lease: unknown; policy: unknown } | { mode: 'replay'; responseBody: unknown; responseStatus: number | null };
...
// La concesión (AT-009): la fila más el testigo con el que sólo su dueño puede cerrarla.
const LEASE = { record: { id: 'rec-1', scope: 'POST /p' }, ownerToken: 'tok-1' };
const POLICY = { operation: 'mutation', storeResponse: true };
...
describe('IdempotencyInterceptor', () => {
  ...
  describe('execute', () => {
    it('ejecuta el handler y persiste el resultado antes de emitir el body', async () => {
      ...
      expect(runtime.completeIdempotency).toHaveBeenCalledWith(LEASE, 201, { ok: true });
    ...
    it('si el handler falla, marca la clave como fallida y propaga el error original', async () => {
      ...
      expect(runtime.failIdempotency).toHaveBeenCalledWith(LEASE);
    ...
    it('usa el tenant del usuario autenticado y el método+ruta como scope', async () => {
      ...
      expect(runtime.claimIdempotency).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          // La huella la calcula el servicio (AT-010); el interceptor entrega la petición tal cual.
          request: { body: undefined, query: undefined, params: undefined },
        }),
      );
    });
