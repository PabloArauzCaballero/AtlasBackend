...
|---|---|---|---|
| Directorio de destinatarios de clientes | `RemoteRecipientDirectoryAdapter` devuelve `unsupported` | no existe contrato HTTP con Clientes; sin él, el piloto no puede enviar a clientes | contrato HTTP de `RecipientDirectoryPort` (bloqueo del GO) |
...
| Configuración completa del monolito | `env.ts` importado por cada dependencia | la carga por capacidad (AT-046) no se aplicó a todas las dependencias de Mensajería | extender `loadCapabilityConfig` |
