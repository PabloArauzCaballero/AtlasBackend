...
describe('WhatsappAdapter', () => {
  ...
  it('normalize (verificado) marca reachable/otp/phone y score alto', async () => {
    const obs = await adapter.normalize({
      status: 'OTP_VERIFIED',
      payload: { status: 'OTP_VERIFIED', whatsappReachable: true, phoneMatch: true, contactabilityScore: 0.96 },
    } as never);
  ...
  it('normalize (no alcanzable, score bajo) exige revisión manual', async () => {
    const obs = await adapter.normalize({
      ...
      payload: { status: 'NOT_REACHABLE', whatsappReachable: false, phoneMatch: false, contactabilityScore: 0.1 },
    } as never);
    expect(obs.find((o) => o.observationKey === 'whatsapp_phone_match')).toMatchObject({ verified: false, manualReviewRequired: true });
    expect(obs.find((o) => o.observationKey === 'whatsapp_contactability_score')).toMatchObject({
      verified: false,
