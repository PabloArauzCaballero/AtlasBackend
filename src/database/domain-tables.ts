/**
 * @file El inventario de qué tabla vive en qué schema de dominio.
 ...
 * natural: arrastraba consigo las funciones de resolución —que sí son cortas y estables— por
 * encima de las 300 líneas de `check:file-size`. Es la misma separación que ya se hizo entre el
 * registro de modelos y el módulo de Nest que los cablea: aquí el inventario, allí la regla.
 *
 * `check:domain-schema-layout` compara esta lista contra las tablas REALES de PostgreSQL, así que
 * una entrada nueva sin su migración deja el gate en rojo.
 */
