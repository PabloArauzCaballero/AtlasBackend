...
describe('TelcoGenericAdapter', () => {
  ...
  it('execute lanza en disabled; escenario high vs normal cambia el riesgo de SIM swap', async () => {
    ...
    expect((await adapter.execute(local('fraud_signal_high'))).payload).toMatchObject({ simSwapRiskLevel: 'HIGH', recentSimChangeDetected: true });
  });

  it('normalize (LOW, score alto) verifica el owner y no exige revisión', async () => {
    const obs = await adapter.normalize({ status: 'VERIFIED', payload: { phoneNumberActive: true, lineAgeDays: 720, lineAgeBucket: 'OLD', simSwapRiskLevel: 'LOW', ownerMatchScore: 0.9, manualReviewRequired: false } } as never);
    expect(obs.find((o) => o.observationKey === 'phone_number_owner_match_score')).toMatchObject({ valueNumber: 0.9, verified: true, manualReviewRequired: false });
  ...
  it('normalize (HIGH, score bajo) exige revisión manual y no verifica el owner', async () => {
    const obs = await adapter.normalize({ status: 'VERIFIED', payload: { simSwapRiskLevel: 'HIGH', ownerMatchScore: 0.42, manualReviewRequired: true } } as never);
    expect(obs.find((o) => o.observationKey === 'phone_number_owner_match_score')).toMatchObject({ verified: false, manualReviewRequired: true });
  ...
  it('normalize sin lineAgeDays -> STRING con DATA_NOT_AVAILABLE', async () => {
    ...
    expect(obs.find((o) => o.observationKey === 'phone_line_age_days')).toMatchObject({ valueType: 'STRING', valueString: 'DATA_NOT_AVAILABLE', verified: false });
  });
});
