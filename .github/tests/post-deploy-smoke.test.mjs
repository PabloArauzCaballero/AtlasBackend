...
async function runScenario({ ready = true, commit = sha } = {}) {
  ...
  const server = createServer((request, response) => {
    ...
    if (request.url === '/api/v1/health/liveness') {
      response.end(JSON.stringify({ data: { status: 'alive' } }));
    } else if (request.url === '/api/v1/health/readiness') {
      response.statusCode = ready ? 200 : 503;
      response.end(JSON.stringify({ data: { status: ready ? 'ready' : 'not_ready' } }));
    } else if (request.url === '/api/v1/health') {
      response.end(JSON.stringify({ data: { service: 'atlas-backend', version: '1.2.3', commit, status: 'ok' } }));
    } else {
  ...
  try {
    ...
    const child = spawn(process.execPath, ['scripts/post-deploy-smoke.mjs'], {
      env: { ...process.env, SMOKE_BASE_URL: `http://127.0.0.1:${address.port}`, TARGET_SHA: sha, SMOKE_ATTEMPTS: '1' },
    });
    let stderr = '';
...
test('acepta live, ready y versión del SHA validado', async () => {
  ...
  assert.deepEqual(paths, ['/api/v1/health/liveness', '/api/v1/health/readiness', '/api/v1/health']);
});
