...
export class WhatsappAdapter implements ExternalProviderAdapter {
  ...
  async execute(request: ExternalProviderExecutionInput): Promise<ExternalProviderRawResult> {
    ...
    // `WHATSAPP_GENERIC_REAL_INTEGRATION_IMPLEMENTED` sea honesto: es una bandera de entorno que un operador puede
    ...
    if (request.mode === 'production' || request.mode === 'sandbox') throw new Error('WHATSAPP_GENERIC_REAL_INTEGRATION_NOT_CONFIGURED');
