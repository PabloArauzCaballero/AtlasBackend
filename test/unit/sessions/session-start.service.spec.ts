...
describe('SessionStartService.startSession', () => {
  ...
  describe('telemetría opcional del arranque (ambos lados de cada ?? null)', () => {
    async function arrange(flow: unknown) {
      ...
      (mocks.sessionsRepository.findLatestOnboardingFlow as jest.Mock).mockResolvedValue(flow as never);
      return mocks;
    }

    it('con snapshot, SIM e IP completos persiste cada bloque con sus valores y liga el flow', async () => {
      const mocks = await arrange({ id: 'flow-1' });
    ...
    it('con los bloques presentes pero vacíos, cada campo opcional cae a null (y sin flow, el id queda en null)', async () => {
      const mocks = await arrange(null);
      ...
      await mocks.service.startSession({
        ...
        context,
        body: baseBody({
          device: { deviceFingerprintHash: 'fp-1', fingerprintVersion: 'v1', channel: 'mobile_app', snapshot: {} },
          permissions: [{ permissionCode: 'camera', granted: false }],
          simObservation: {},
          ipReputation: {},
        }) as never,
      });
